-- Fix payment_status to return 'fully_paid' instead of 'paid'
-- This ensures consistency with UI components and fully_paid_students view
-- 
-- Issue: Function returns 'paid' but system expects 'fully_paid'
-- Impact: 
--   - Payment page won't show "Fully Paid" badge
--   - Fully Paid Students admin page won't show any students
--   - Status mismatch across the system
...
BEGIN
  ...
  IF v_schedule_exists THEN
    BEGIN
      -- CRITICAL FIX: Multiple checks to ensure deposit is excluded
      SELECT COALESCE(SUM(amount), 0)
      INTO v_total_due
      FROM public.contract_payment_schedule
      WHERE contract_id = v_contract_id
        AND LOWER(COALESCE(label, '')) NOT LIKE '%deposit%'
        AND (sequence > 1 OR amount != v_deposit_amount); -- Exclude if sequence 1 AND matches deposit
  ...
  -- Get total paid from installment payments ONLY (exclude deposits with multiple checks)
  BEGIN
    ...
    WHERE student_application_id = p_application_id
      AND payment_type = 'instalment'  -- CRITICAL: Only instalment type
    ...
    IF COALESCE(v_total_paid, 0) = 0 OR v_total_paid IS NULL THEN
      ...
      WHERE student_application_id = p_application_id
        ...
        -- CRITICAL: Multiple checks to exclude deposits
        AND (
  ...
  RETURN QUERY SELECT 
    ...
    v_last_payment_date,
    CASE 
      -- CRITICAL FIX: Return 'fully_paid' instead of 'paid' for consistency
