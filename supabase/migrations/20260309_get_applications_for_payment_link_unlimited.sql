-- Remove 300-row limit from applications list for manual payment link (staff)
-- so older confirmed/custom applications are also available in the dropdown.
-- Keeps existing filters and ordering identical, only removes LIMIT.
...
COMMENT ON FUNCTION public.get_applications_for_payment_link() IS
  'Returns applications (id, student_name, student_email, contract_slug) for manual payment link dropdown; status in confirmed, awaiting_signature, awaiting_deposit. No row limit.';
