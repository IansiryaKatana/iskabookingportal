-- Add booking sources: Unity Sales, HFS Sales (staff-created applications and bulk import).

-- 1. Update CHECK constraint on student_applications.booking_source
...
ALTER TABLE public.student_applications
  ...
  CHECK (booking_source IS NULL OR booking_source IN (
    'rebooker', 'website', 'imported', 'partner_referral', 'unity_sales', 'hfs_sales'
  ));

COMMENT ON COLUMN public.student_applications.booking_source IS 'How the booking was acquired: rebooker, website, imported, partner_referral, unity_sales, hfs_sales.';

-- 2. Update bulk_import_student_applications to accept unity_sales and hfs_sales
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      -- Parse and validate booking_source (rebooker, website, imported, partner_referral, unity_sales, hfs_sales)
