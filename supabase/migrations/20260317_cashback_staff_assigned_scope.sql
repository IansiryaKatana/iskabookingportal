-- Add 'staff_assigned' scope for cashback campaigns and update eligibility logic

-- 1) Extend applies_to enum constraint to include 'staff_assigned'
ALTER TABLE public.cashback_campaigns
DROP CONSTRAINT IF EXISTS cashback_campaigns_applies_to_check;

ALTER TABLE public.cashback_campaigns
ADD CONSTRAINT cashback_campaigns_applies_to_check
CHECK (applies_to IN ('all', 'new', 'rebooking', 'staff_assigned'));

-- 2) Update check_cashback_eligibility to treat 'staff_assigned' as manually applied but eligible
...
BEGIN
  ...
  -- Ensure only one cashback per application
  IF EXISTS (
