-- Add booking_source support to bulk_import_student_applications (rebooker, website, imported, partner_referral).
-- When booking_source = 'rebooker', set is_rebooking = true and rebooking_reason.
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      -- Parse and validate booking_source (rebooker, website, imported, partner_referral)
      v_booking_source := NULLIF(LOWER(TRIM(v_row->>'booking_source')), '');
      IF v_booking_source IS NOT NULL AND v_booking_source NOT IN ('rebooker', 'website', 'imported', 'partner_referral') THEN
