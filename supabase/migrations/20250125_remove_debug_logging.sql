-- Remove debug logging from database functions
-- This removes all INSERT INTO debug_logs statements from functions

-- Remove debug logging from get_fully_paid_students
...
-- Note: We keep the debug_logs table and get_debug_logs function in case they're needed for future debugging
-- But all functions no longer write to it, so it won't grow unnecessarily

COMMENT ON FUNCTION public.get_fully_paid_students(UUID, UUID, TEXT, TEXT) IS 
'Returns fully paid students with optional filtering by contract, academic year, and date range.
