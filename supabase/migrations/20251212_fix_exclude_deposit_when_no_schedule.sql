-- Fix get_payment_summary when contract_payment_schedule doesn't exist
-- When there's no schedule in the database, calculate from payment_plan_installments
-- This ensures the total_due matches the actual installments that were created/paid
--
-- Issue: When schedule doesn't exist, function uses contract_total - deposit = 9126
-- But actual installments sum to 9027.01, causing remaining_balance = 98.99
--
-- Fix: When schedule is empty, calculate from payment_plan_installments (excluding deposit)
...
BEGIN
  ...
  -- If no schedule exists OR schedule sum was 0, calculate from payment_plan_installments
  -- This ensures total_due matches the actual installments that were created
  -- CRITICAL: Calculate from REMAINING BALANCE, not contract total
  IF (NOT v_schedule_exists OR COALESCE(v_total_due, 0) = 0) AND v_payment_plan_id IS NOT NULL THEN
    BEGIN
      ...
      WHERE payment_plan_id = v_payment_plan_id
        ...
        -- The payment_plan_installments table also has a deposit row that must be excluded
        AND LOWER(COALESCE(label, '')) NOT LIKE '%deposit%';
