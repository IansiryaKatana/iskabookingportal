-- Stop treating deposit_payment_intent_id alone as a succeeded Stripe deposit.
-- Only stripe_payments (succeeded/completed) and manual_payments are authoritative.
...
COMMENT ON VIEW public.unified_payment_history IS
  'Unified payment history (Stripe + manual). Deposits require a succeeded stripe_payments row or manual_payments entry — deposit_payment_intent_id alone is not treated as paid.';
