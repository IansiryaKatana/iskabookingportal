-- Third fix for get_installment_breakdown:
-- Ensure the returned sequence column is INTEGER to match the function signature,
-- by casting any SMALLINT sequence columns to INTEGER in both branches.
...
BEGIN
  ...
  IF v_payment_plan_id IS NOT NULL AND v_installment_base > 0 THEN
    ...
    SELECT
      j.installment_id,
      j.sequence::INTEGER AS sequence,
      j.label,
      j.due_date,
