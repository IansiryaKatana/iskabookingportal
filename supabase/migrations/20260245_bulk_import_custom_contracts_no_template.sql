-- Applications with custom contracts: no template contract required.
-- Resolve academic_year_id from academic_year_name; studio_grade_id from studio_number (if present)
-- or from first segment of contract_slug (e.g. silver, gold, platinum). Create custom contract + plan per row.
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      v_ay_name_normalized := NULLIF(TRIM(v_row->>'academic_year_name'), '');
      IF v_ay_name_normalized IS NULL THEN
        RAISE EXCEPTION 'academic_year_name is required';
      END IF;

      -- Resolve academic year by name only (no template contract)
      SELECT id INTO v_academic_year_id FROM public.academic_years WHERE TRIM(name) = v_ay_name_normalized LIMIT 1;
      IF v_academic_year_id IS NULL THEN
        RAISE EXCEPTION 'Academic year % not found.', v_row->>'academic_year_name';
      END IF;

      -- Resolve studio_grade_id: from studio_number if provided, else from first segment of contract_slug (e.g. silver, gold)
      v_studio_grade_id := NULL;
      IF v_row->>'studio_number' IS NOT NULL AND v_row->>'studio_number' != '' THEN
        SELECT s.studio_grade_id INTO v_studio_grade_id FROM public.studios s WHERE s.studio_number = TRIM(v_row->>'studio_number') LIMIT 1;
      END IF;
      IF v_studio_grade_id IS NULL AND v_row->>'contract_slug' IS NOT NULL AND v_row->>'contract_slug' != '' THEN
        v_slug_normalized := REPLACE(TRIM(v_row->>'contract_slug'), '/', '-');
        v_grade_slug := split_part(v_slug_normalized, '-', 1);
        IF v_grade_slug != '' THEN
          SELECT id INTO v_studio_grade_id FROM public.studio_grades WHERE slug = v_grade_slug LIMIT 1;
        END IF;
      END IF;
      IF v_studio_grade_id IS NULL THEN
        RAISE EXCEPTION 'Could not resolve studio grade: provide studio_number or use a contract_slug whose first part matches a studio grade slug (e.g. silver, gold, platinum).';
      END IF;

      -- Skip if already imported (student has a custom-import contract for this academic year)
      v_existing_id := NULL;
      IF p_skip_existing THEN
        ...
        JOIN public.contracts c ON c.id = sa.contract_id
          ...
          AND c.source_contract_id IS NULL
          AND c.slug LIKE 'custom-import-%'
        WHERE sa.student_id = v_student_id
        LIMIT 1;
        IF v_existing_id IS NOT NULL THEN
          RETURN QUERY SELECT v_row_num, 'skipped'::TEXT, v_existing_id, 'Application already exists for this student and academic year (custom import)'::TEXT;
        END IF;
      END IF;
      ...
      -- 1) Create custom payment plan from instalment_due_dates / instalment_amounts
      ...
      -- 2) Create custom contract (no template: source_contract_id = NULL)
      ...
      IF v_row->>'studio_number' IS NOT NULL AND v_row->>'studio_number' != '' THEN
        SELECT s.id INTO v_studio_id FROM public.studios s WHERE s.studio_number = TRIM(v_row->>'studio_number') LIMIT 1;
      END IF;
...
'Bulk import applications with custom contracts. No template contract required: academic_year from academic_year_name, studio_grade from studio_number or first segment of contract_slug (e.g. silver, gold). Creates one custom contract + fixed instalment plan per row.';
