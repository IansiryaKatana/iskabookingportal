-- Per-instalment payment breakdown for a single application.
-- Non-destructive: new RPC only, existing functions and views unchanged.
--
-- For the selected application, returns one row per non-deposit instalment in the
-- contract_payment_schedule that is relevant to this application's current plan/schedule.
-- When the application has selected_payment_plan_id, amounts and due dates are computed
-- from the plan (same logic as useStudentPayments / ManualPaymentDialog).
-- Otherwise, amounts come from contract_payment_schedule.amount.
--
-- amount_paid is the sum of stripe + manual instalment payments that explicitly reference
-- this instalment_id. We do not reallocate payments across instalments here to avoid
-- changing semantics of existing data.
--
-- payment_status is derived per instalment:
--   - 'unpaid'   when amount_paid = 0
--   - 'partial'  when 0 < amount_paid < amount_due - tolerance
--   - 'paid'     when amount_paid >= amount_due - tolerance
--
-- This RPC is intended for admin and portal UIs; reports can consume it later if needed.
...
BEGIN
  ...
  -- Derive contract total base (same as get_payment_summary)
  BEGIN
    SELECT
  ...
  -- Plan-based amounts when selected_payment_plan_id exists, else schedule.amount
  IF v_payment_plan_id IS NOT NULL AND v_installment_base > 0 THEN
    ...
    non_deposit_schedule AS (
      SELECT *
      FROM schedule_rows
      WHERE LOWER(COALESCE(label, '')) NOT LIKE '%deposit%'
    ),
    first_n_schedule AS (
      SELECT *
    ...
    payments AS (
      SELECT
        COALESCE(sp.metadata->>'instalment_id', mp.instalment_id::text) AS instalment_id_text,
  ...
  ELSE
    -- No plan selected: use contract_payment_schedule amounts directly (excluding deposits)
    RETURN QUERY
