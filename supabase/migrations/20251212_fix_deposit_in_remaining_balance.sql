-- Fix: Deposit showing in remaining balance when all payments are paid
-- Issue: Deposit is being counted in total_due or total_paid incorrectly
-- Fix: Ensure deposits are completely excluded from installment calculations
...
BEGIN
  ...
  IF v_schedule_exists THEN
    BEGIN
      ...
      -- 1. Check label doesn't contain 'deposit'
      -- 2. Check sequence > 1 (deposits are usually sequence 1)
      -- 3. Check amount doesn't match deposit amount (safety check)
