-- Force fix duplicate get_fully_paid_students function
-- This script aggressively removes ALL versions and recreates a single one
...
DECLARE
  func_record RECORD;
BEGIN
  -- Find all versions of get_fully_paid_students
  FOR func_record IN
    SELECT 
      p.proname,
  ...
  LOOP
    -- Drop each version
    EXECUTE format('DROP FUNCTION IF EXISTS public.get_fully_paid_students(%s) CASCADE', func_record.args);
    RAISE NOTICE 'Dropped function: get_fully_paid_students(%)', func_record.args;
  END LOOP;
END;
$$;
...
BEGIN
  ...
  IF func_count = 0 THEN
    RAISE EXCEPTION 'Function get_fully_paid_students was not created!';
  ELSIF func_count > 1 THEN
    RAISE EXCEPTION 'Multiple versions of get_fully_paid_students still exist: % (signatures: %)', func_count, func_list;
  ELSE
    RAISE NOTICE 'Success: Only one version of get_fully_paid_students exists';
  END IF;
END;
$$;
