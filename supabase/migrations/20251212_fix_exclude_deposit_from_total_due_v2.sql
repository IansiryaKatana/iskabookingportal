-- Fix get_payment_summary to exclude deposits from total_due calculation (V2 - More Robust)
-- The contract_payment_schedule includes deposits (sequence 1, label "Deposit")
-- When summing amounts, we must exclude deposits to get the correct installment total
-- 
-- Issue: Total Due was showing £9,126.00 (including £99.00 deposit)
-- But installments only sum to £9,027.01
-- Remaining Balance incorrectly showed £98.99 instead of £0.00
--
-- Fix: Filter out deposits when summing contract_payment_schedule amounts
-- Use BOTH label check AND sequence check for maximum reliability
...
BEGIN
  ...
  -- CRITICAL FIX: Exclude deposits from the sum - only sum installments
  -- Deposits are stored in contract_payment_schedule with label "Deposit" (sequence 1)
  -- Use BOTH label check AND sequence check for maximum reliability
  BEGIN
    ...
    WHERE contract_id = v_contract_id
      -- Exclude deposits: deposits have label "Deposit" (case-insensitive)
  ...
  IF COALESCE(v_total_due, 0) = 0 AND v_payment_plan_id IS NOT NULL THEN
    BEGIN
      ...
      WHERE payment_plan_id = v_payment_plan_id
        -- Exclude deposit installment (label "Deposit")
        AND LOWER(COALESCE(label, '')) NOT LIKE '%deposit%';
...
DO $$
BEGIN
  RAISE NOTICE 'Function get_payment_summary has been updated to exclude deposits from total_due calculation';
END $$;
