-- Relax contract lookup for bulk_import_applications_custom_contracts: normalize slug (trim + slash to hyphen)
-- so CSV slugs like "silver-48-weeks-25/26" match DB slugs like "silver-48-weeks-25-26".
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      -- Normalize slug and academic year for flexible matching (CSV may use 25/26, DB may use 25-26; trim spaces)
      v_slug_normalized := REPLACE(TRIM(v_row->>'contract_slug'), '/', '-');
      v_ay_name_normalized := NULLIF(TRIM(v_row->>'academic_year_name'), '');

      -- Resolve template contract: match normalized slug and academic year name (template = no student_application_id)
      ...
      JOIN public.academic_years ay ON ay.id = c.academic_year_id
        AND TRIM(ay.name) = v_ay_name_normalized
      WHERE REPLACE(TRIM(c.slug), '/', '-') = v_slug_normalized
        AND c.student_application_id IS NULL
      LIMIT 1;
      IF v_source_contract_id IS NULL THEN
        RAISE EXCEPTION 'Contract with slug % for academic year % not found (template contract). Check that a template contract exists with that slug (try normalizing 25/26 to 25-26) and academic year name matches exactly.', v_row->>'contract_slug', v_row->>'academic_year_name';
      END IF;
...
'Bulk import applications with custom contracts. Contract slug matched after normalizing: trim and replace / with - (e.g. 25/26 and 25-26 match).';
