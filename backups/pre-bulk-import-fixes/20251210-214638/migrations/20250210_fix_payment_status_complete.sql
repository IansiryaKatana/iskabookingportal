-- COMPLETE FIX for ambiguous payment_status column reference
-- The issue: payment_status exists in RETURN TABLE and also in the query
-- Solution: Use a completely different alias in the LATERAL join
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(payment_data.remaining_balance, COALESCE(pr.total_contract_value, 0)) AS remaining_balance,
    COALESCE(payment_data.payment_status_value, 'unpaid') AS payment_status,
  ...
  LEFT JOIN LATERAL (
    SELECT 
      ...
      payment_summary.remaining_balance,
      payment_summary.payment_status::TEXT AS payment_status_value,
      payment_summary.last_payment_date
    FROM public.get_payment_summary(sa.id) payment_summary
