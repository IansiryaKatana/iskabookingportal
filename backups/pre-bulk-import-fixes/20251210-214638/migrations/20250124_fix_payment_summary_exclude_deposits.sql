-- Fix get_payment_summary to exclude deposits from total_paid calculation
-- Deposits are separate from installments and should not count towards installment total
-- Only installment payments should be counted towards total_paid for installment calculations
...
BEGIN
  ...
  -- Deposits are separate and should not be included in installment total_paid
  BEGIN
    SELECT 
    ...
    WHERE student_application_id = p_application_id
      ...
      -- Exclude deposits - only count installment payments
      -- Include if: payment_type = 'instalment' OR has installment_number
      -- Exclude if: payment_type = 'deposit' OR type = 'deposit' in metadata
      AND (
