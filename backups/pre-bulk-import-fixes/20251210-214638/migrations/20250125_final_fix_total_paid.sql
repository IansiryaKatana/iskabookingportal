-- FINAL FIX: Simplify and make the function more robust
-- This version directly queries stripe_payments first, then falls back to view
...
BEGIN
  ...
  -- CRITICAL FIX: Query stripe_payments DIRECTLY first (most reliable)
  -- Then fall back to unified_payment_history if needed
  BEGIN
  ...
  RETURN QUERY SELECT 
    ...
    v_total_paid,
    GREATEST(v_total_due_after_cashback - v_total_paid, 0) AS remaining_balance,
    COALESCE(v_payment_count, 0),
    v_last_payment_date,
    CASE 
      WHEN v_total_paid >= (v_total_due_after_cashback - 0.01) AND v_total_due_after_cashback > 0.01
        THEN 'fully_paid'
