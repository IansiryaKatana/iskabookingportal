-- Fix get_payment_summary to include 'succeeded' status payments
-- Stripe payments use 'succeeded' status, but the function was only checking for 'completed'
-- This prevents fully paid students from showing up when they've paid via Stripe
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.student_applications WHERE id = p_application_id) THEN
    -- Return zeros if application doesn't exist
  ...
  -- This handles Pay in Full and other cases where schedule hasn't been generated
    ...
    BEGIN
      ...
      IF v_payment_plan_id IS NOT NULL 
        ...
        BEGIN
          ...
          -- Sum up installments (for percentage-based, calculate from total; for fixed, use amount directly)
          SELECT COALESCE(SUM(
            CASE 
...
-- Update comment
COMMENT ON FUNCTION public.get_payment_summary(UUID) IS 'Returns payment summary for an application, accounting for cashback discounts. Handles Pay in Full plans by calculating from payment_plan_installments when contract_payment_schedule is empty. Includes both succeeded and completed payment statuses.';
